...
function requireAnymatch() {
  ...
  const matchPatterns = (patterns, negPatterns, args, returnIndex) => {
    ...
    const path = normalizePath2(_path, false);
...
function requireChokidar() {
  ...
  class WatchHelper {
    constructor(path, watchPath, follow, fsw) {
      ...
      this.globFilter = this.hasGlob ? anymatch2(path, void 0, ANYMATCH_OPTS) : false;
    ...
    add(paths_, _origAdd, _internal) {
      ...
      if (cwd) {
        paths = paths.map((path) => {
          ...
          if (disableGlobbing || !isGlob2(path)) {
            return absPath;
          }
    ...
    */
    ...
    _isIgnored(path, stats) {
      ...
      if (!this._userIgnored) {
        ...
        const paths = arrify(ignored).filter((path2) => typeof path2 === STRING_TYPE && !isGlob2(path2)).map((path2) => path2 + SLASH_GLOBSTAR);
    ...
    _getWatchHelpers(path, depth) {
      const watchPath = depth || this.options.disableGlobbing || !isGlob2(path) ? path : globParent2(path);
...
function requireMain() {
  ...
  (function(module) {
    ...
    const chokidar2 = requireChokidar();
    const fs = require$$0$2;
    ...
    module.exports = function elecronReload(glob, options = {}) {
      ...
      const watcher = chokidar2.watch(glob, Object.assign({ ignored: [ignoredPaths, mainFile] }, options));
      ...
      if (eXecutable) {
        ...
        const hardWatcher = chokidar2.watch(mainFile, Object.assign({ ignored: [ignoredPaths] }, options));
